...
#' @param ext.vars Character string of external predictor variable names across which equal weighting will be applied.
...
marginal_avg <- function(marginal_preds, target.vars, ext.vars = NULL,
                         wt = c("iso", "joint", "all"),
                         interval = c(.05, .95),
                         verbose = TRUE){
  require(data.table)
  ...
  if(!is.null(ext.vars)) peripheral_vars <- peripheral_vars[!(peripheral_vars %in% ext.vars)]

  if(verbose){
    message("Calculating weighted averages treating ", paste(peripheral_vars, collapse = ", "), " as peripheral variables, and ", paste(ext.vars, collapse = ", "), " as unweighted external variables.\n")
