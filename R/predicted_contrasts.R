...
#' @param target.vars Character string of the name of the predictor for which to calculate contrasts
...
#' @examples
#' \dontrun{}
predicted_contrasts <- function(marginal_preds, target.vars,
                               interval = c(.05, .95),
                               verbose = FALSE){
  ...
  if(length(levs) > 2){
    ...
    avg_contrast_df <- wide_dt[, .(mean = mean(contrast),
      ...
      as.data.frame()
    names(avg_contrast_df)[names(avg_contrast_df) == "mean"] <- paste(levs[2], levs[1], "contrast", sep = "_")
  }
