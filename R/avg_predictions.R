...
#' @param wt Character string indicating the type of weighting to be applied to the calculation of mean predictions. See details below. Possible values are \code{'iso'} (default), \code{'joint'}, \code{'all'}, \code{'none'}.
...
avg_predictions <- function(marginal_preds, target.vars, equal.wt = NULL,
  ...
  # get the averages
  if(wt == "none"){
    mar_avg_df <- mar_table[, .(mean_pred = mean(get(pred_outcome), na.rm = TRUE)),
