...
For this vignette we'll the following basic libraries. *Note that currently the package only works with random forest models fit with the `{ranger}` package.* 
...
Here we exclude `Genre` from the weighting.

```{r}
marginal_avg(marginal_preds, "Possessor_Animacy3", ext.vars = "Genre", verbose = FALSE)
```
...
marginal_avg(marginal_preds, c("Possessor_Animacy3", "Possessor_NP_Type"), verbose = F) |> 
  ggplot(aes(x = Possessor_Animacy3, y = mean_prob_s, color = Possessor_NP_Type)) +
  geom_hline(yintercept = .5, color = "grey", linetype = "dashed") +
...
The overall picture is not much different, but the resolution is finer. 
