...
Plotting interactions fo continuous predictors is not as simple, but can be done. Here the effects of `Possessor_Length` and `Possessor_Thematicity` are largely independent of one another. 
...
The output of `predicted_contrasts()` is a data frame, where the last column `proper_common_contrast` reports the obtained difference in predicted probabilities (i.e. the predicted contrasts) for `Possessor_NP_Type == 'proper'` -- `Possessor_NP_Type == 'common'`.
...
Weighted averages for contrasts can also be computed with `avg_contrasts()`. This is useful for summarizing the patterns over different levels of secondary predictors. Here we calculated the weighted average contrasts for `Possessor_NP_Type` by `Genre`.
