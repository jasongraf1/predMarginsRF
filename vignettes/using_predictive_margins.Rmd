...
For this vignette we'll load the following basic libraries. 
...
We'll start with classification cases first, then look at an example with a continuous outcome.

# Predicting disctrete outcomes (classification)

In the case of categorical outcomes, our predictions will be the probability of one or more classes.

For this tutorial, we'll use actual datasets from linguistic research. We start with the `written_genitives` dataset of English genitive constructions (*the children's voices* [*s*-genitive] vs. *the voices of the children* [*of*-genitive]) included in the package and discussed in Sönning & Grafmiller (2022). See `?written_genitives` for more on the dataset.

```{r}
glimpse(written_genitives)
```

We're interested in exploring the features that correlate with use of different constructions. We know, for instance, that animate (i.e. human) possessors tend to co-occur more often with the *s*-genitive, and inanimate possessors more with the *of*-genitive.
...
Define the formula using some contextual features known to correlate with the use of genitive constructions, e.g. GENRE, ANIMACY, NOUN TYPE (proper vs. common), LENGTH (in words), THEMATICITY (text frequency) of the possessor, and FINAL SIBILANT (whether the possessor phrase ends in a sibilant sound).
...
Now we try a forest with the `{party}` package. Again, in a real case we would tune the `mtry` and other hyperparameters. Note that `{party}` forests run much more slowly than `{ranger}`.
...
Once again, evaluate the model. We can get the class-specific predicted probabilities with `treeresponse()` and then bind them into a dataframe. This also takes a long time, so we won't run it here, but the code is included below for illustration.
...
`tree_preds_ranger` is a list. The dataframe with the predicted probabilities is contained in `tree_preds_ranger$predictions`.
...
See `help("tree_predictions")` for more information.


### Average predictions for a categorical predictor

First we'll calculate predictive margins for the targeted variable `Possessor_Animacy3`. All other predictors in the model (i.e. `Genre`, `Possessor_Animacy3`, `Possessor_NP_Type`, `Possessor_Length` `Possessor_Thematicity`, `Final_Sibilant`) are therefore peripheral variables, which means that we need to average over their levels/values. As discussed in Sönning & Grafmiller (2022), we can use different weighting schemes when averaging over peripheral predictors. There, a distinction is made between external and internal variables. For external variables, is seems sensible to form simple averages; for internal variables, we would usually prefer weighted averages, with weights reflecting the observed distribution of the predictor in the data.

The function `avg_predictions()` takes two arguments, the model and a character vector of the names of the targeted variable(s) (`target.vars = "..."`) in the model for which to obtain predictive margins. Additional optional arguments include:

- `equal.wt`: Character string of (external) predictor variable names across which equal weighting (i.e. simple averages) will be applied.
- `wt`: Character string indicating the type of weighting to be applied to those peripheral predictors **not** specified in `ext.vars`. 
- `interval`: The lower and upper quantiles for the percentile interval summarizing the distribution of predicted probabilities across the trees in the forest. Default is `c(.05, .95)`, for a 90% percentile interval.
- `verbose`: If `TRUE` additional information will be printed about the dataset.

The default weighting is the isolated weighting (`"iso"`), where the weights are based on the expected distribution of all combinations of the peripheral variables (i.e. those **not** `Possessor_Animacy3` and **not** specified in the `ext.vars` argument), calculated from the independent marginal (overall) distribution of each peripheral predictor in the dataset.
...
We can plot these and compare. Here, the differences are negligible, but they can vary (considerably) in some cases.
...
If we want more values for this predictor, we'll need to recalculate the tree predictions, setting the `break` argument to a different set of numbers.
...
Multivariate associations among predictor variables and the outcome (i.e. "interaction effects") can be calculated by simply including them in the vector of target variables.
...
Plotting interactions fo continuous predictors is not as simple, but can be done. Here the effects of LENGTH and THEMATICITY are largely independent of one another. 
...
We refer to "predicted contrasts" as the difference between two predicted outcomes for specific values (levels) of a predictor. For the targeted variable `Possessor_NP_Type`, which has two levels (common vs. proper nouns), the predicted contrast is the difference between the predicted probabilities for the two levels. For instance, if the predicted probabilities are .55 for common nouns and .70 for proper nouns, the predicted contrast is +.15. These contrasts are estimated for every combination of predictor values, averaged over all the trees in the forest. 

### Predictive contrasts for binary variables

For binary targeted variables, which level is subtracted from which depends on the coding of the factor. By default, R orders factor levels alphabetically. For `Possessor_NP_Type`, `common` is therefore represented (internally) as `1`, and `proper` as `2`. For binary predictors, the first level (`common`) is subtracted from the second (`proper`).

The function `predicted_contrasts()` takes two arguments, the model and a character vector of the names of the targeted variable(s) (`target.vars = "..."`) in the model for which to obtain predicted contrasts.

The output of `predicted_contrasts()` is a data frame, where the last column `mean_Possessor_NP_Type_contrast` reports the obtained difference in predicted probabilities (i.e. the predicted contrasts).
...
median_cont <- median(np_type_contrasts$mean_Possessor_NP_Type_contrast)
...
median_cont <- median(np_type_contrasts_party$mean_Possessor_NP_Type_contrast)
...
### Predicted contrasts for categorical variable with +3 levels

The calculation of predictive contrasts for categorical predictors with more than two levels works similarly. The output of `predicted_contrasts()` is again a data frame, but now there is a set of final columns, one for each contrast. The targeted variable `Possessor_Animacy3`, for instance, has 3 levels, so there are three possible contrasts. Accordingly, the data frame includes three contrast columns: `contrast_animate_collective`, `contrast_animate_inanimate`, and `contrast_collective_inanimate`. The naming is such that the predicted probability for the second level is subtracted from the first. So for `contrast_animate_collective`, the prediction for `collective` is subtracted from that for `animate`. A predicted contrast of +.20 therefore signals that the predicted probability (of *s*-genitives) is higher for `animate` compared to `collective` referents.
...
We can plot these and color by the comparisons with Press (previous work has shown a distinct difference between Press and the rest).

```{r fig.width=6}
genre_contrasts |> 
  pivot_longer(6:15, names_to = "contrast") |> 
  mutate(
    IsPress = if_else(grepl("Press", contrast), "press", "nonpress")
  ) |> 
  ggplot(aes(value, contrast, fill = IsPress)) +
  geom_density_ridges() +
...
So here we are interested, e.g., in the association of things like word FAMILIARITY and subject AGE with response times for identifying words. 
...
The outcome in our model will the the mean response time latency `RTlexdec` for the words in the dataset.
...
Now get the tree predictions for the models. 
...
```{r}
median_cont <- median(age_contrasts$mean_AgeSubject_contrast)
age_contrasts |> 
  ggplot(aes(mean_AgeSubject_contrast)) +
