...
4. Summarize the effects of one or more target predictor by averaging over all other predictors, where the averages are weighted by the distributions of the specific predictor combinations in the original dataset.

Steps 2 and 3 are executed in `tree_predictions()`. The other three functions provide different methods for summarizing patterns of specific target predictors.
...
We start with classification cases first, then look at an example with a continuous outcome.
...
The resulting object `tree_preds_ranger` is a list. The dataframe with the predicted probabilities is contained in `tree_preds_ranger$predictions`.
...
Now try the joint weighting. This weighting uses the observed numbers of each combination in the original training dataset.
