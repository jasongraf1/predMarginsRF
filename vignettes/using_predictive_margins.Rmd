...
First we'll calculate predictive margins for the targeted variable `Possessor_Animacy3`. All other predictors in the model (i.e. `Genre`, `Possessor_Animacy3`, `Possessor_NP_Type`, `Possessor_Length` `Possessor_Thematicity`, `Final_Sibilant`) are therefore peripheral variables, which means that we need to average over their levels/values. Sönning & Grafmiller (2022) discuss different weighting schemes that can be used when averaging over peripheral predictors. By default, weighted averages are used for all peripheral variables.
...
- `interval`: The lower and upper quantiles for the percentile interval summarizing the distribution of predicted probabilities across the trees in the forest. Default is `c(.05, .95)`, for a 90% interval.
- `verbose`: If `TRUE` additional information will be printed about the dataset.
...
Plotting interactions of 2 or more continuous predictors is not as simple, but can be done. Plotting the interaction of `Possessor_Length` and `Possessor_Thematicity`, for instance, suggests that their effects are largely independent of one another. 
...
For example, for the targeted variable `Possessor_NP_Type`, which has two levels (common vs. proper nouns), the Predictive contrast is the difference between the predicted probabilities for the two levels. For instance, if the predicted probabilities are .55 for common nouns and .70 for proper nouns, the predicted contrast is +.15. These contrasts are estimated for every combination of predictor values, averaged over all the trees in the forest. 
...
The output of `predicted_contrasts()` is a data frame, with one column, here `proper_common_contrast`, reporting the obtained difference in predicted probabilities (i.e. the predicted contrasts) for the target predictor, and `lower` and `upper` columns reflecting the bounds of the percentile interval. In this case, contrasts represent the difference: `Possessor_NP_Type == 'proper'` -- `Possessor_NP_Type == 'common'`. The naming is such that the predicted outcome for the second level is subtracted from the first.
...
These can be plotted easily.

```{r fig.width=7}
avg_contrasts(tree_preds_ranger, "Possessor_NP_Type", by = c("Genre", "Possessor_Animacy3" ), 
              wt = "joint", verbose = F) |> 
  ggplot(aes(Possessor_Animacy3, mean_proper_common_contrast)) +
  geom_pointrange(aes(ymin = lower, ymax = upper)) +
  coord_flip() +
  facet_wrap(~Genre) +
  labs(x = "Difference in predicted s-genitive probability\n(proper - common)", 
       y = "") +
  ggtitle("Possessor NP type across animacy levels and genres")
  theme_classic()
```
